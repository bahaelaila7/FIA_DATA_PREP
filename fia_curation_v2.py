...
def sql_base_measurements(con):
    ...
    SELECT
        ...
        C.CONDID,
        P.MEASYEAR, P.MEASMON, P.MEASDAY,
...
def sql_clean_subplot_years(con):
    ...
    SELECT STATECD, UNITCD, COUNTYCD, PLOT, INVYR, SUBP, MEASYEAR, MEASDATE
    FROM base_subplot_cond
    GROUP BY STATECD, UNITCD, COUNTYCD, PLOT, INVYR, SUBP, MEASYEAR, MEASDATE
...
def sql_longitudinal_subplots(con):
    ...
    SELECT STATECD, UNITCD, COUNTYCD, PLOT, SUBP,
           ...
           MIN(MEASYEAR) AS first_year,
           MAX(MEASYEAR) AS last_year
...
def analyze_spcd_confusion(df: pl.DataFrame) -> tuple[pl.DataFrame, pl.DataFrame]:
    ...
    # --- Detect transitions ---
    transitions = (
        df.select(TREE_ID + ["MEASYEAR", "SPCD"])
    ...
    # Count SPCD occurrences per tree, with latest MEASYEAR as tie-breaker
    ...
    spcd_counts = (
        ...
        .agg([
            pl.len().alias("_n_obs"),
            pl.col("MEASYEAR").max().alias("_latest"),
        ])
        .sort(["_n_obs", "_latest"])
...
def handle_reconcile5(df: pl.DataFrame) -> pl.DataFrame:
    ...
    if n_r5 == 0:
        return df.with_columns([
            ...
            pl.lit(None).cast(pl.Int32).alias("reconcile5_first_year"),
            pl.lit(False).alias("reconcile5_recovered"),
        ])

    # Most-recent non-null DIA before each row (within tree, sorted by MEASYEAR)
    ...
    df = df.sort(TREE_ID + ["MEASYEAR"]).with_columns(
    ...
    # Recovery flag: back to RECONCILECD∈{1,2} after the first RECONCILECD=5 year
    # reconcile5_first_year: the first MEASYEAR this tree dropped below threshold
    first_r5_year = (
        df.filter(r5)
        .group_by(TREE_ID)
        .agg(pl.col("MEASYEAR").min().alias("reconcile5_first_year"))
    )
    df = df.join(first_r5_year, on=TREE_ID, how="left")
    df = df.with_columns(
        (
            pl.col("reconcile5_first_year").is_not_null() &
            pl.col("RECONCILECD").is_in([1, 2]) &
            (pl.col("MEASYEAR") > pl.col("reconcile5_first_year"))
...
def propagate_totage(df: pl.DataFrame,
    ...
    anchors = _select_best_anchor(sources, tree_bounds)
...
def detect_condition_resets(df: pl.DataFrame) -> pl.DataFrame:
    ...
    Returns a DataFrame of (COND_ID + MEASYEAR) marking each reset event,
    ...
    # --- 1. STDAGE resets ---
    cond_traj = (
        df.select(COND_ID + ["MEASYEAR", "cond_stdage", "cond_stdorgcd",
                              "TRTCD1", "TRTCD2", "TRTCD3",
                              "DSTRBCD1", "DSTRBCD2", "DSTRBCD3"])
        .unique(subset=COND_ID + ["MEASYEAR"])
        .sort(COND_ID + ["MEASYEAR"])
        .with_columns([
            pl.col("cond_stdage").shift(1).over(COND_ID).alias("_prev_stdage"),
            pl.col("MEASYEAR").shift(1).over(COND_ID).alias("_prev_year"),
        ])
    )
    ...
    # Keep all reasons per condition-year (a reset can have multiple signals)
    reset_reasons = (
        all_resets.group_by(COND_ID + ["MEASYEAR"])
        .agg(pl.col("reset_reason").unique().sort().alias("reset_reasons"))
    )
    # Flat flag for joining
    reset_flag = (
        reset_reasons.select(COND_ID + ["MEASYEAR"])
        .unique()
...
def assign_stdage(df: pl.DataFrame,
    ...
    then propagate: age = first_STDAGE + (MEASYEAR - first_MEASYEAR).
    ...
    # Attach per-measurement-year reset flag
    df = df.join(
        reset_flag.rename({"MEASYEAR": "_reset_year"}),
        left_on=COND_ID + ["MEASYEAR"],
        right_on=COND_ID + ["_reset_year"],
        how="left",
    ).with_columns(pl.col("is_reset").fill_null(False))

    # cond_ever_reset: condition had at least one reset in any year
    ever_reset_conds = (
    ...
    # First reset year per condition (earliest year a reset was detected)
    # Kept as stdage_cond_first_reset_year for downstream traceability.
    first_reset_per_cond = (
        reset_flag.group_by(COND_ID)
        .agg(pl.col("MEASYEAR").min().alias("stdage_cond_first_reset_year"))
    )
    df = df.join(first_reset_per_cond, on=COND_ID, how="left")

    # Tree's first measurement year across all conditions
    # Kept as stdage_tree_first_measyear for traceability.
    tree_first_meas = (
        df.group_by(TREE_ID)
        .agg(pl.col("MEASYEAR").min().alias("stdage_tree_first_measyear"))
    )
    df = df.join(tree_first_meas, on=TREE_ID, how="left")

    # Last year this tree was observed alive — used to distinguish cut/removed
    # pre-reset trees from carry-forward survivors.
    tree_last_alive = (
        df.filter(pl.col("STATUSCD") == 1)
        .group_by(TREE_ID)
        .agg(pl.col("MEASYEAR").max().alias("stdage_tree_last_alive_year"))
    )
    df = df.join(tree_last_alive, on=TREE_ID, how="left")

    # Eligibility for confirmed_reset conditions (three-way split):
    #
    #   POST-RESET DEBUT (first_measyear >= reset_year):
    #     New cohort tree. Post-reset STDAGE = individual age. ✓ eligible
    #
    #   PRE-RESET, DID NOT SURVIVE (last_alive_year <= reset_year):
    ...
    #
    #   PRE-RESET, SURVIVED RESET (first_measyear < reset_year AND
    #                               last_alive_year > reset_year):
    ...
    pre_reset = (
        ...
        pl.col("stdage_cond_first_reset_year").is_not_null() &
        (pl.col("stdage_tree_first_measyear") < pl.col("stdage_cond_first_reset_year"))
    )
    survived_reset = (
        pl.col("stdage_tree_last_alive_year").is_not_null() &
        (pl.col("stdage_tree_last_alive_year") > pl.col("stdage_cond_first_reset_year"))
    )
    post_reset_debut = (
        pl.col("cond_ever_reset") &
        pl.col("stdage_cond_first_reset_year").is_not_null() &
    ...
    cand_std = (
        ...
        .select(TREE_ID + ["MEASDATE", "MEASYEAR", "cond_stdorgcd",
                            pl.col("cond_stdage").cast(pl.Float64).alias("src_age")])
    )

    anchors_std = _select_best_anchor(cand_std, tree_bounds)

    # Retrieve per-tree anchor year and origcd by joining on (TREE_ID, anchor_date)
    anchor_meta = (
        cand_std
        .join(
        ...
        .select(TREE_ID + [
            pl.col("MEASYEAR").alias("_anchor_year"),
    ...
    if n_reset > 0:
        reset_cond_years = (
            ...
            .select(COND_ID + ["MEASYEAR"]).unique()
            .join(reset_reasons, on=COND_ID + ["MEASYEAR"], how="left")
        )
        # Explode reset_reasons and count
        reason_counts = (
            reset_cond_years.explode("reset_reasons")
            .filter(pl.col("reset_reasons").is_not_null())
            .group_by("reset_reasons").len()
            .sort("len", descending=True)
        )
        print(f"         reset reasons (condition-years):")
    ...
    df = df.rename({
        "_anchor_year":    "stdage_anchor_year",
    ...
    df = df.with_columns(
        pl.when(
            pl.col("stdage_anchor_year").is_not_null() &
            pl.col("stdage_cond_first_reset_year").is_not_null() &
            (pl.col("stdage_anchor_year") >= pl.col("stdage_cond_first_reset_year"))
        )
        .then(pl.lit(True))
        .otherwise(pl.lit(False))
...
def _compute_tree_bounds(df: pl.DataFrame,
    ...
    tree_first = (
        ...
        .agg(
            ...
            pl.col("MEASYEAR").min().alias("_first_year"),
        )
    )
    sub_visits = (
        subplot_years
        .select(SUBPLOT_ID + ["MEASYEAR", "MEASDATE"])
        .unique(subset=SUBPLOT_ID + ["MEASYEAR"])
        .rename({"MEASYEAR": "_sub_year", "MEASDATE": "_sub_measdate"})
    )
    prev_sub = (
        tree_first
        .join(sub_visits, on=SUBPLOT_ID, how="left")
        .filter(pl.col("_sub_year") < pl.col("_first_year"))
        .group_by(TREE_ID)
        .agg(pl.col("_sub_measdate").max().alias("prev_sub_measdate"))
    )
    return tree_first.drop("_first_year").join(prev_sub, on=TREE_ID, how="left")
...
def _select_best_anchor(candidates: pl.DataFrame,
    ...
    2. Apply a mandatory min-age shift so that age at first_measdate >= 1.
    3. Compute potential max-age violation: implied age at the most recent
       subplot visit before first_measdate (after shift).
    ...
    min_shift      : years added to satisfy the age >= 1 constraint
...
def process_estage(df: pl.DataFrame, estage_raw: pl.DataFrame,
    ...
    anchors_est = _select_best_anchor(cand_est, tree_bounds)
...
def build_subplot_year_scaffold(con) -> pl.DataFrame:
    ...
    SELECT
        ...
        BSC.INVYR, BSC.MEASYEAR, BSC.MEASDATE,
...
def build_snapshots(df: pl.DataFrame, scaffold: pl.DataFrame) -> pl.DataFrame:
    ...
    tree_agg = (
        ...
        .group_by(
            SUBPLOT_INVYR + ["MEASYEAR", "SPCD", "SPGRPCD",
    ...
    snapshots_with = (
        ...
        .join(tree_agg, on=SUBPLOT_INVYR + ["MEASYEAR"], how="left")
    ...
    snapshots = snapshots.sort(SUBPLOT_ID + ["MEASYEAR", "SPCD", "estimated_age"])
...
def diagnostics(df: pl.DataFrame, snapshots: pl.DataFrame):
    ...
    all_with_stdage = (
        df.select(["STATECD", "UNITCD", "COUNTYCD", "PLOT", "CONDID",
                   "MEASYEAR", "cond_stdage", "cond_stdorgcd"])
        .unique(subset=["STATECD", "UNITCD", "COUNTYCD", "PLOT", "CONDID", "MEASYEAR"])
        .filter(pl.col("cond_stdage").is_not_null() & (pl.col("cond_stdage") > 0))
        .sort(["STATECD", "UNITCD", "COUNTYCD", "PLOT", "CONDID", "MEASYEAR"])
    )
    ...
    if all_with_stdage.height > 0:
        consec = all_with_stdage.with_columns([
            ...
            pl.col("MEASYEAR").shift(-1).over(cond_id_cols).alias("next_year"),
        ]).filter(pl.col("next_stdage").is_not_null())

        consec = consec.with_columns([
            (pl.col("next_year") - pl.col("MEASYEAR")).alias("interval"),
    ...
    if empty.height > 0:
        ...
        if n_unexplained > 0:
            ...
            live_years = (
                ...
                .group_by(SUBPLOT_ID)
                .agg([
                    pl.col("MEASYEAR").min().alias("first_live_year"),
                    pl.col("MEASYEAR").max().alias("last_live_year"),
                ])
            )
            unexp_detail = (
                unexplained.select(SUBPLOT_ID + ["MEASYEAR"])
                .join(live_years, on=SUBPLOT_ID, how="left")
                .with_columns([
                    pl.col("last_live_year").is_not_null().alias("ever_had_trees"),
                    (
                        pl.col("last_live_year").is_not_null() &
                        (pl.col("last_live_year") < pl.col("MEASYEAR"))
                    ).alias("had_prior_live_trees"),
                    (
                        pl.col("first_live_year").is_not_null() &
                        (pl.col("first_live_year") > pl.col("MEASYEAR"))
    ...
    for a, b, tag in pairs:
        ...
        # pre-reset anchor (stdage_anchor_year < tree's first MEASYEAR would be
        ...
        #
        # Proxy: stdage_anchor_year == tree's overall first MEASYEAR (from
