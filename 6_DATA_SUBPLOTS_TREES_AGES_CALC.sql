...
SELECT T.STATECD, T.UNITCD, T.COUNTYCD, T.PLOT, T.SUBP, T.subp_num_meas, T.subp_has_dstrb, T.first_dstrb_measdate, T.TREE, T.num_meas,
  T.TREE_CNS,PREV_TRE_CNS,PLT_CNS,INVYRs,CONDIDS,AGES,SPCDs,SPGRPCDs,CCLCDs,AGENTCDs,TPA_UNADJs,dias,  stockings, TOTAGEs, STDAGEs, STDORGCDs, DRYBIO_AGs, HTs,measdates,STATUSCDs,STANDING_DEAD_CDs,RECONCILECDs,
  T.SPCD, T.SPGRPCD, T.SPECIES_SYMBOL, T.sftwd_hrdwd, T.GENUS, ages_calc_uncorrected, ages_calc_with_totage, ages_calc_with_stdage, age_calc_correction,
...
FROM (
  ...
  SELECT T.STATECD, T.UNITCD, T.COUNTYCD, T.PLOT, T.SUBP, T.subp_num_meas, T.subp_has_dstrb, T.first_dstrb_measdate, T.TREE, T.num_meas,
    ...
    array_agg(T.age_calc + (vv.measdate - T.age_measdate ) /(365.25) ORDER BY vv.invyr) ages_calc_uncorrected,
    BOOL_AND(T.age_calc_with_totage) ages_calc_with_totage,
    BOOL_AND(T.age_calc_with_stdage) ages_calc_with_stdage
  ...
  FROM (
    ...
    SELECT TT.*, 
      ...
      (totage_idx is null AND stdage_idx is not null) age_calc_with_stdage,
      (CASE WHEN totage_idx is not null then totages[totage_idx] ELSE (CASE WHEN stdage_idx is not null then stdages[stdage_idx] ELSE ages[age_idx] END) END) age_calc,
      (CASE WHEN totage_idx is not null then measdates[totage_idx] ELSE (CASE WHEN stdage_idx is not null then measdates[stdage_idx] ELSE measdates[age_idx] END) END) age_measdate,
      (CASE WHEN totage_idx is not null then invyrs[totage_idx] ELSE (CASE WHEN stdage_idx is not null then invyrs[stdage_idx] ELSE invyrs[age_idx] END) END) age_invyr,
      SP.GENUS, SP.SPECIES_SYMBOL, SP.sftwd_hrdwd
    FROM (
      SELECT *, 
        totage_idx_non_null[array_upper(totage_idx_non_null,1)] totage_idx,
        stdage_idx_non_null[array_upper(stdage_idx_non_null,1)] stdage_idx,
      ...
      FROM (
        SELECT DST.*, 
          ...
          (SELECT array_agg(sub.idx) FROM generate_subscripts(DST.totages,1) sub(idx)
          WHERE DST.STDAGEs[sub.idx] is not null)  stdage_idx_non_null, 

          (SELECT array_agg(sub.idx) FROM generate_subscripts(DST.ages,1) sub(idx)
          WHERE DST.ages[sub.idx] is not null) age_idx_non_null

        FROM DATA_SUBPLOTS_TREES DST
        --WHERE 
    ...
    T.TREE_CNS,PREV_TRE_CNS,PLT_CNS,INVYRs,CONDIDS,AGES,SPCDs,SPGRPCDs,CCLCDs,AGENTCDs,TPA_UNADJs,dias,stockings,TOTAGEs, STDAGEs, STDORGCDs,DRYBIO_AGs, HTs,measdates,STATUSCDs,STANDING_DEAD_CDs,RECONCILECDs,T.SPCD, T.SPGRPCD, T.SPECIES_SYMBOL, T.sftwd_hrdwd, T.GENUS
