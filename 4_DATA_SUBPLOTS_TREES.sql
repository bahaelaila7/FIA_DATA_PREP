...
array_agg((CASE WHEN C.STDAGE < 1 THEN NULL ELSE C.STDAGE END) ORDER BY T.INVYR) STDAGEs,
array_agg(C.STDORGCD ORDER BY T.INVYR) STDORGCDs,
...
array_agg(T.CCLCD ORDER BY T.INVYR) CCLCDs,
array_agg(T.AGENTCD ORDER BY T.INVYR) AGENTCDs,
...
JOIN COND C ON C.PLT_CN = T.PLT_CN AND T.CONDID = C.CONDID
