...
WITH Grouped_COHORTS AS (
  SELECT v.PLT_CN, T.STATECD, T.UNITCD, T.COUNTYCD, T.PLOT,T.SUBP, v.measdate, T.SPECIES_SYMBOL, T.SPGRPCD, T.SFTWD_HRDWD, T.subp_has_dstrb, T.first_dstrb_measdate,
          CAST ( round(v._age_calc) AS INTEGER) age_calc, 
          sum(v.drybio_ag*v.TPA_UNADJ)*0.112085 AGB, 
          sum(v.TPA_UNADJ) tree_count, 
          bool_and(STDORGCD=1) as planted_stand
  FROM DATA_SUBPLOTS_TREES_AGES_CALC T
  CROSS JOIN LATERAL unnest(T.PLT_CNs, T.drybio_ags, T.statuscds, T.ages_calc, T.measdates, T.TPA_UNADJs, T.STDORGCDs)
  v(PLT_CN, drybio_ag,statuscd, _age_calc, measdate, TPA_UNADJ, STDORGCD)
  WHERE v.statuscd = 1 and v._age_calc is not null and v.drybio_ag is not null and v.TPA_UNADJ > 0.0
  GROUP BY v.PLT_CN, T.STATECD, T.UNITCD, T.COUNTYCD, T.PLOT,T.SUBP, v.measdate, T.SPECIES_SYMBOL, T.SPGRPCD, T.SFTWD_HRDWD, T.subp_has_dstrb, T.first_dstrb_measdate, age_calc, stdorgcd
),
