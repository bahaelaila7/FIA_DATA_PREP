...
Measurements AS (
SELECT PLT_CN, STATECD, UNITCD, COUNTYCD, PLOT, min(measdate) first_measdate, count(distinct measdate) plot_meas_num
FROM Grouped_COHORTS
GROUP BY PLT_CN, STATECD, UNITCD, COUNTYCD, PLOT)
